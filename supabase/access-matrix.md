...
| `public.profiles` | Owner reads own profile; shared username reads for report/comment surfaces; admin reads role data | Owner updates own profile fields; server-owned points updates | Server-fetched role/identity helpers | Authorization MUST use DB role data, never `user_metadata`. Public username joins still block a strict RLS rollout here. |
